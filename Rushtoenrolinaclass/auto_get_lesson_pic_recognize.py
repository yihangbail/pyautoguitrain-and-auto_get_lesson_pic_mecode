...
# 定义鼠标事件
#click_times 点击次数   retry_times 循环点击次数
def Mouse(click_times, img_name, retry_times):
    if retry_times == 1:  # 只循环点击一次
        #locateCenterOnScreen定位图片的中心坐标；参数confindence是控制图像的置信程度，范围[0,1]
    ...
    elif retry_times == -1:  # 一直循环点击
        while True:
            location = pyautogui.locateCenterOnScreen(img_name,confidence=0.9)
            if location is not None:
                pyautogui.click(location.x, location.y, clicks=click_times, duration=0.2, interval=0.2)
    elif retry_times > 1: # 循环点击次数为retry_tiemes
        i = 1
